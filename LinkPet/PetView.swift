...
struct PetView: View {
    ...
    var body: some View {
        ZStack {
            ...
            CatSpriteView(state: viewModel.state, frame: viewModel.frame)
                .scaleEffect(x: viewModel.facingRight ? 1 : -1, y: 1)
                .frame(width: 80, height: 80)
                .onTapGesture { viewModel.onPetTapped() }
                .contextMenu {
...
struct CatSpriteView: View {
    ...
    var currentEmoji: String {
        switch state {
        case .idle:    return frame % 2 == 0 ? "🐱" : "😺"
        case .walk:    return ["🐈", "🐱", "🐈", "😺"][frame % 4]
        case .sit:     return "🐈‍⬛"
        case .happy:   return ["😻", "🥰", "😻", "🎉"][frame % 4]
        case .eating:  return ["😺", "🐟", "😸", "🐟"][frame % 4]
        case .sleeping:return frame % 2 == 0 ? "😴" : "💤"
        }
    }
}
